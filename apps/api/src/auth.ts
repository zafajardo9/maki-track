...
const apiUrl = process.env.MAKI_API_URL || "http://localhost:1337";
...
function getPasswordResetSubject(locale?: string | null) {
  const normalized = locale?.toLowerCase();
  if (normalized?.startsWith("de")) return "Maki-Passwort zurücksetzen";
  if (normalized?.startsWith("vi")) return "Đặt lại mật khẩu Maki";
  if (normalized?.startsWith("ja")) return "Maki のパスワードをリセット";
  return "Reset your Maki password";
}
...
function getDeviceAuthClientIds(): Set<string> {
  ...
  return new Set(["maki-cli", "maki-mcp"]);
}
...
export const auth = betterAuth({
  ...
  plugins: [
    organization({
      ...
      // is not verified from accepting/rejecting an invitation. Maki does not
      // verify emails on signup, so leaving the default on breaks invitation
      // acceptance for everyone. The invitation link id is the actual secret
      // here, so gate on that rather than on email verification.
      ...
      organizationHooks: {
        ...
        afterCreateOrganization: async ({ organization, user }) => {
          ...
          // in `@maki/permissions`; admins can later replace them in the
      ...
      async sendInvitationEmail(data) {
        const inviteLink = `${process.env.MAKI_CLIENT_URL}/invitation/accept/${data.id}`;
        ...
        if (
          ...
          console.warn(
            "Invitation created but email not sent due to email delivery (Resend) not being configured",
          );
          return;
        }
      },
  ...
  rateLimit: {
    // Enable in cloud; self-hosted instances opt in by setting MAKI_CLOUD.
  ...
  hooks: {
    before: createAuthMiddleware(async (ctx) => {
      // Block invite-member calls on cloud to disposable-email addresses.
      // The 2026-05-28 incident saw ~14k phishing invites sent from
      // throwaway disposable-email signups; gating here shuts that path
      // off without affecting self-hosted instances.
      ...
      const [userCountRow] = await db
        .select({ value: count() })
        .from(schema.userTable);
      const isInstanceAdminSetup = (userCountRow?.value ?? 0) === 0;

      // Cloud-only abuse gates on signup. Self-hosted instances leave
      // MAKI_CLOUD/TURNSTILE_SECRET_KEY unset and skip both.
      if (isCloud() && !isInstanceAdminSetup) {
        ...
        if (signupEmail && isDisposableEmail(signupEmail)) {
          throw new APIError("BAD_REQUEST", {
            message: "Sign-up with disposable email addresses is not allowed.",
          });
        }
