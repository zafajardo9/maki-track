...
export function createApp() {
  ...
  if (!corsOrigins && !reflectUnconfiguredOrigins) {
    console.warn(
      "[cors] Neither CORS_ORIGINS nor MAKI_CLIENT_URL is set, so cross-origin requests are refused. Same-origin deployments (the bundled image) are unaffected; set MAKI_CLIENT_URL if the web app is served from another origin.",
    );
  }
  ...
  api.get("/openapi", (c) => {
    const document = api.getOpenAPI31Document({
      ...
      info: {
        ...
        version: "1.0.0",
        description:
          "Maki Project Management API - Manage projects, tasks, labels, and more",
      },
      servers: [
        {
          url: normalizeApiServerUrl(
            process.env.MAKI_API_URL || "https://cloud.kaneo.app",
          ),
          description: "Maki API Server",
        },
      ],
      security: [{ bearerAuth: [] }],
    });
  ...
  api.use("*", async (c, next) => {
    ...
    return Sentry.withIsolationScope(async () => {
      ...
      try {
        ...
        const windowId = c.req.header("X-Maki-Window-Id");
  ...
  app.route(
    "/",
    mcpWellKnownRoutes(
      (process.env.MAKI_API_URL || "http://localhost:1337").replace(
        /\/api\/?$/,
        "",
      ),
    ),
...
export async function startServer(
  ...
  const server = serve(
    ...
    () => {
      console.log(
        `⚡ API is running at ${process.env.MAKI_API_URL || "http://localhost:1337"}`,
      );
    },
  );
