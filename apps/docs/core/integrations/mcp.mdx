...
Both the HTTP endpoint and stdio package expose these tools:
...
The Streamable HTTP endpoint additionally exposes project tag tools:

- Tags: `list_project_tags`, `create_tag`, `attach_tag_to_task`, `detach_tag_from_task`, `delete_tag`

Unlike workspace labels, a palette tag can be deleted through MCP — the blast
radius is one project.
